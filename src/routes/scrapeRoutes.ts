...
export const createScrapeRoutes = (scrapeQueue: Queue) => {
    // Initialize services
    ...
    // Define routes
    scrapeRoutes.post('/scrape', zValidator('json', ScrapeJobSchema), (c) => 
        scrapeController.createScrapeJob(c)
    );

    scrapeRoutes.get('/scrape/:id', (c) => 
        scrapeController.getScrapeJob(c)
    );

    scrapeRoutes.get('/scrape', (c) => 
        scrapeController.getAllScrapeJobs(c)
    );
