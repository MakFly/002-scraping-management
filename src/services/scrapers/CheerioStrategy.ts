...
export class CheerioStrategy implements ScrapeStrategy {
  ...
  public async scrape(job: ExtendedScrapeJob): Promise<ScrapedData> {
    ...
    try {
      ...
      if (!domainConfig) {
        throw new Error(
          `Aucune configuration trouvée pour le domaine: ${job.source}`
        );
      }
      ...
      for (let currentPage = 1; currentPage <= pageCount; currentPage++) {
        ...
        logger.info(
          `Scraping de la page ${currentPage}/${pageCount} pour ${job.source}: ${url}`
        );
        ...
        logger.info(`HTML récupéré pour la page ${currentPage}/${pageCount}`);
        ...
        if (currentPage === 1) {
          ...
          logger.info(`Titre de la page: ${pageTitle}`);
        }

        let pageItems: ScrapedItem[] = [];
        // Traitement spécial pour eBay
        if (job.source.includes("ebay")) {
          pageItems = this.extractEbayItems($, domainConfig.selectors);
          logger.info(`${pageItems.length} items trouvés sur la page ${currentPage}/${pageCount} pour eBay`);
        } else {
          // Extraction standard pour les autres sites
          pageItems = this.extractItems($, domainConfig.selectors);
          logger.info(`${pageItems.length} items trouvés sur la page ${currentPage}/${pageCount}`);
        }
        
        allItems = [...allItems, ...pageItems];
        logger.info(`Total d'items trouvés jusqu'à présent: ${allItems.length}`);

        // Si c'est la dernière page, on sort de la boucle
        if (currentPage === pageCount) {
          logger.info(`Dernière page (${currentPage}/${pageCount}) atteinte, fin du scraping`);
          break;
        }
        ...
        if (job.source.includes("autoscout24")) {
          ...
          logger.info(
            `AutoScout24: préparation de la page ${currentPage + 1}/${pageCount}`
          );

          // Ajouter un petit délai entre les requêtes pour éviter de surcharger le serveur
          if (currentPage < pageCount) {
            const delay = 1500 + Math.random() * 1000;
            logger.info(`Pause de ${Math.round(delay)}ms avant la prochaine page`);
        ...
        if (
          ...
          !domainConfig.selectors.nextPage ||
          !$(domainConfig.selectors.nextPage).length
        ) {
          break;
        }
      ...
      //   logger.warn(
      //     `Scraping Cheerio pour ${job.source} n'a pas obtenu de résultats satisfaisants, JavaScript pourrait être requis`
      //   );
      ...
      logger.info(
        `Scraping Cheerio terminé pour ${job.source}, ${allItems.length} items trouvés sur ${pageCount} page(s)`
      );
      ...
      logger.error(
        `Erreur lors du scraping Cheerio pour ${job.source}: ${error instanceof Error ? error.message : String(error)}`
      );
      throw error;
    }
  }
  ...
  private buildUrl(
    ...
    if (domain.includes("autoscout24")) {
      ...
      if (!query && domainConfig?.options?.defaultSearchUrl) {
        ...
        logger.info(
          `Utilisation de l'URL par défaut pour AutoScout24: ${defaultUrl}`
        );
        return defaultUrl;
      }
      ...
      if (domainConfig?.options?.baseUrl) {
        logger.info(
          `Utilisation de baseUrl pour AutoScout24: ${domainConfig.options.baseUrl}`
        );
   ...
   */
  private extractItems($: cheerio.CheerioAPI, selectors: any): ScrapedItem[] {
    ...
    $(selectors.container).each((i, el) => {
      ...
      if (selectors.url) {
        ...
        if (item.url && !item.url.startsWith("http")) {
          item.url = new URL(
            item.url,
            $("base").attr("href") || "https://" + selectors.domain
          ).toString();
        }
      }
   ...
   */
  private cleanPrice(priceText: string): string | number {
   ...
   */
  private looksLikeJavaScriptRequired(
    $: cheerio.CheerioAPI,
    html: string
  ): boolean {
